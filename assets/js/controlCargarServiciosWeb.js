import {
    formatoPNG, modificarURLServicioWMS, validarUrl, mensaje,
    proyeccion3857, centroide3857, buscarCapaPorNombre,
} from './configuracion';
...
function limpiar() {
    ...
    conectarServicioWeb.innerHTML = '<i class="bi bi-box-arrow-down"></i> Conectar';
...
limpiarServicioWeb.addEventListener('click', () => {
    ...
    global.vista.setZoom(global.mapa.getSize()[0] > 1296 ? 6 : 5.4);
    limpiar();
});
...
conectarServicioWeb.addEventListener('click', () => {
    if (direccionServicioWeb.value.length > 0) {
        conectarServicioWeb.innerHTML = '<span class="spinner-grow spinner-grow-sm" aria-hidden="true"></span> Conectando';
...
function conectarServicioOGCWMS() {
    const urlServicioExternoWMS = modificarURLServicioWMS(direccionServicioWeb.value.trim());
    ...
    if (validarUrl(urlServicioExternoWMS)) {
        fetch(urlServicioExternoWMS, { mode: 'cors' })
            ...
            .catch(error => {
                mensaje('mensajeServicioWeb', `<strong>Error:</strong> ${error}`, 'danger');
                limpiar();
            });
...
agregarCapas.addEventListener('click', () => {
    if (capasTematicas.options.length > 0) {
        ...
        const nombreCapa = capasTematicas.options[selectedIndex].innerText.substring(0, 25);
        const url = direccionServicioWeb.value;
        const tipo = tipoServicioWeb.value;

        const optionToRemove = capasTematicas.querySelector(`option[value="${idCapa}"]`);
...
function conectarServicioRestArcGIS() {
    ...
    //capasTematicas.removeChild(select.options[0]);

    fetch(`${urlArcGISRest}?f=json`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: new URLSearchParams({ url: urlArcGISRest })
    })
        ...
        .then(l => {
            ...
            if (l.estado) {
                ...
                new Toast(`Respuesta inesperada del servidor, ${l.mensaje}`);
            }
        })
        .catch(e => {
            console.log(e);
